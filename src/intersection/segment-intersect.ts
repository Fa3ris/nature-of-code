...
export const segmentIntersect = (p5: p5) => {
  ...
  p5.setup = () => {
    ...
    const intersectInfos = LineLineDetector.detectSegmentSegment(
      segment.start,
      segment.end,
      segment2.start,
      segment2.end
    ...
    if (intersectInfos) {
      ...
      const intersectionOnSegment1 =
        intersectInfos.ratio1 >= 0 && intersectInfos.ratio1 <= 1;
      const intersectionOnSegment2 =
        intersectInfos.ratio2 >= 0 && intersectInfos.ratio2 <= 1;

      offScreen.text(
        `intersection is${intersectionOnSegment1 ? "" : " not"} on segment 1`,
        10,
        10
      );
      offScreen.text(
        `intersection is${intersectionOnSegment2 ? "" : " not"} on segment 2`,
        10,
        10 + offScreen.textSize()
      );
      offScreen.text(
        `intersection is${intersectionOnSegment2 ? "" : " not"} on segment 2`,
        10,
      ...
      offScreen.text(
        `x: ${intersectInfos.point1.x.toFixed(
          2
        )} y: ${intersectInfos.point1.y.toFixed(2)}`,
        10,
        10 + 2 * offScreen.textSize()
      );
    }
  };

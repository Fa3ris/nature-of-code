...
export const lineIntersect = (p5: p5) => {
  ...
  p5.setup = () => {
    ...
    const intersect = LineLineDetector.detectLineLine(
      segment.start,
      segment.end,
      segment2.start,
      segment2.end
    ...
    if (intersect) {
      ...
      if (overFlowX || overFlowY) {
        ...
        const interSegment = LineLineDetector.intersectInsideSegment(
          intersect,
          segment.start,
          segment.end
        );
        const interSegment2 = LineLineDetector.intersectInsideSegment(
          intersect,
          segment2.start,
          segment2.end
        );
        ...
        offScreen.text(
          `intersection point falls on ${count} segment${count > 1 ? "s" : ""}`,
          10,
          10
        );
      ...
      offScreen.text(
        `x: ${intersect.x.toFixed(2)} y: ${intersect.y.toFixed(2)}`,
        10,
